...
# NOTE: Added werkzeug.security for dummy password check
from werkzeug.security import generate_password_hash, check_password_hash 

# ==================== TEMPORARY HARDCODED USERS ====================
# TEMPORARY FIX to bypass Supabase user table issue (PGRST116)
# AUTHORIZED_USERS_TEMP = {
#     "recruiter1": {"password": "recruit_secure_pwd", "role": "recruitment"},
#     "recruiter2": {"password": "team_pass_123", "role": "recruitment"},
#     "guest_user": {"password": "guest_pass", "role": "guest"},
# }
# REQUIRED_ROLE = "recruitment"
RECRUIT_ROLE = "recruitment"
ADMIN_ROLE = "admin"

AUTHORIZED_USERS_TEMP = {
    "recruiter1": {"password": "recruit_secure_pwd", "role": RECRUIT_ROLE},
    "recruiter2": {"password": "team_pass_123", "role": RECRUIT_ROLE},
    "guest_user": {"password": "guest_pass", "role": "guest"},
    "admin_user": {"password": "admin_secure_pwd", "role": ADMIN_ROLE} # <-- ADDED ADMIN
}
# ===================================================================

# def login_required(f):
#     """
#     Decorator to protect views, redirecting unauthorized users to the login page.
#     """
#     @wraps(f)
#     def decorated_function(*args, **kwargs):
#         # Allow access if user is logged in AND has the required role
#         if session.get('user_role') == REQUIRED_ROLE:
#             return f(*args, **kwargs)
#         flash("Access denied. Please log in with an authorized recruitment account.", "error")
#         return redirect(url_for('login', next=request.url))
#     return decorated_function
def login_required(f):
    """
    Decorator to protect views, redirecting unauthorized users to the login page.
    Allows access for ADMIN and RECRUITMENT roles.
    """
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # --- MODIFIED ---
        # Allow access if user role is one of the allowed roles
        allowed_roles = [RECRUIT_ROLE, ADMIN_ROLE]
        if session.get('user_role') in allowed_roles:
            return f(*args, **kwargs)
        return redirect(url_for('login', next=request.url))
    return decorated_function
# --- NEW DECORATOR ---
def admin_required(f):
    """
    Decorator to protect views, allowing access ONLY to admin users.
    """
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Allow access only if user is logged in AND has the admin role
        if session.get('user_role') == ADMIN_ROLE:
            return f(*args, **kwargs)
        
        # If they are logged in but not admin, send to index.
...
def log_activity(event_type, details=None):
    """
    Logs an activity event to the Supabase activity_log table.
    Requires 'username' and 'user_role' to be present in the Flask session.
    """
    if not supabase or 'username' not in session:
        # Cannot log if Supabase is down or user is not logged in
        print(f"WARNING: Activity log skipped for {event_type} - DB down or no session.")
        return

    # Fetch project ID if available
    project_id = None
    if 'currentProject' in request.form:
        try:
            current_project = json.loads(request.form.get('currentProject'))
            project_id = current_project.get('id')
        except:
            pass
    
    # Use UTC for logging timestamps
    log_data = {
    ...
    try:
        ...
        print(f"ERROR: Failed to insert activity log for {event_type}: {e}")
...
@app.route('/api/evaluate_candidate', methods=['POST'])
def evaluate_candidate():
    try:
        ...
        # Generate structured evaluation points
        evaluation = generate_evaluation_points(candidate_data)
...
def generate_job_description():
    try:
        ...
        # Prompt for generating a comprehensive job description
        prompt = f"""Create a professional job description for the role of {job_role}. Include:
        1. Brief company overview
        2. Detailed role responsibilities
        3. Required qualifications and skills
        4. Preferred qualifications
        5. Benefits and perks
        Format it in a clear, structured way that's easy to read.
        """
...
def generate_evaluation_points(candidate_data: dict) -> dict:
    """Generate structured evaluation points with justifications
    
    Args:
        candidate_data (dict): Dictionary containing candidate information
        
    Returns:
        dict: Structured evaluation points with justifications
    """
    evaluation = {
        "suggestions": [
            {
                "point": "Focus on DevOps tools and scripting languages",
                "justification": "Current limited experience in these areas needs improvement"
            },
            {
                "point": "Pursue DevOps methodology training",
                "justification": "Will help bridge the gap in direct DevOps experience"
            },
            {
                "point": "Seek cross-team collaboration opportunities",
                "justification": "Will strengthen practical experience in agile environments"
            }
        ],
        "strengths": [
            {
                "point": "Strong AI and data science background",
                "justification": "Demonstrated through AI-powered projects and implementations"
            },
            {
                "point": "Project management and problem-solving",
                "justification": "Evidenced by successful implementation of multiple AI projects"
            },
            {
                "point": "Cloud and containerization proficiency",
                "justification": "Demonstrated through process automation and deployment experience"
            }
        ],
        "improvements": [
            {
                "point": "Limited DevOps tooling experience",
                "justification": "Identified gap in current technical skill set"
            },
            {
                "point": "Need more agile environment exposure",
                "justification": "Limited direct experience with agile methodologies"
            },
            {
                "point": "Cross-functional team collaboration",
                "justification": "Current experience doesn't fully cover dev, QA, and ops interaction"
            }
        ],
        "value_props": [
            {
                "point": "AI innovation potential in DevOps",
                "justification": "Past projects show ability to apply AI in practical scenarios"
            },
            {
                "point": "Complex project delivery capability",
                "justification": "Successfully implemented multiple AI-powered solutions"
            },
            {
                "point": "Technical adaptability",
                "justification": "Experience with cloud platforms and process automation tools"
            }
        ]
    }
    return evaluation
...
def is_resume_file(filename, subject, search_query=""):
    """Checks if a file and subject match resume criteria."""
    
    # --- MODIFIED: Use new variables for clarity ---
    name_lower = (filename or "").lower()
    subject_lower = (subject or "").lower()

    # Original checks
    is_subject_ok = any(kw in subject_lower for kw in RESUME_KEYWORDS)
    is_filename_ok = any(kw in name_lower for kw in RESUME_KEYWORDS)
    
    # --- NEW CHECK: Trust the user's search query ---
    is_query_match = False
    if search_query:
        # Check if the search query appears in the subject or filename
        sq_lower = search_query.lower()
        if sq_lower in subject_lower or sq_lower in name_lower:
             is_query_match = True
    # --- END NEW CHECK ---

    is_excluded = any(kw in name_lower for kw in EXCLUDE_KEYWORDS)
    
    # --- MODIFIED: Add is_query_match to the return logic ---
    return (is_subject_ok or is_filename_ok or is_query_match) and not is_excluded
            ...
            try:
                ...
                for part in mime_msg.walk():
                    ...
                    if filename and filename.lower().endswith('.pdf'):
                        if is_resume_file(filename, subject, search_query):
...
Write a comprehensive, non-repetitive analysis 
( with specific, concrete resume evidence in each sentence), 
combining HR summary and justification. Start with a sub-heading **HR Summary:** and then a sub-heading **Justification:**, and then write the corresponding content for each. 
The **HR Summary:** must be a  3 points, and should mention: domain expertise, technical proficiency, business acumen, teamwork, communication, project/role highlights, and unique strengths combain those things and create a points no need for seperate explaination for each topic in there. Do not repeat phrases or evidence. 
The **Justification:** must be a 3 points and reference project/role/skill evidence from the resume. Each major point must reference different evidence from the resume. Highlight both positive and negative aspects, and address business value, culture fit, and any observable upskilling or future potential  combain those things and create a points no need for seperate explaination for each topic in there, it must be in points dont mention justification in every point.
...
- **Why This Candidate not Suitable:** Reference at least two unique weaknesses or potential concerns from the resume.
...
[{{
    "name": "{name}",
    "ats_score": "[0-100]" "The ats_score should be a precise, calculated value (0-100) based on a detailed match between the resume and the job description. Avoid generic, rounded scores (like 75 or 85) and provide a specific result (e.g., 73, 87).",
    "hr_score": "[1-10]"
}}] 
...
def style_recommendation_subheadings(recommendation):
    ...
    rec_out = re.sub(r"(?<!\*)\s*Why This Candidate not Suitable\s*:(?!\*)", "\n\n**Why This Candidate not Suitable:**", rec_out, flags=re.IGNORECASE)
...
def index():
    try:
        # --- MODIFIED: Pass user_role to the template ---
...
def project_operations(project_id):
    if request.method == 'GET':
        ...
        # Find the project and remove it from the list
        projects = [p for p in projects if p.get('id') != project_id]
            
        # Delete from Supabase
        try:
            # Delete the project record
            response = supabase.table(PROJECTS_TABLE).delete().eq('id', project_id).execute()
            
            # Save the updated projects list
            if save_projects(projects):
                return jsonify({'success': True, 'message': 'Project deleted successfully'}), 200
            else:
                return jsonify({'error': 'Failed to update projects database'}), 500
...
def upload_resume_standalone():
    ...
    # CHANGE: Use getlist to handle multiple files
    files = request.files.getlist('resume') 
    
    if not files: # Check if the list of files is empty
    ...
    # NEW: List to hold all processed candidate data
    candidates = []
    ...
    for file in files: # Iterate over each uploaded file
        ...
        # Ensure a unique filename if multiple files have the same name (optional, but safer)
        unique_filename = f"{str(uuid.uuid4())}_{filename}"
        filepath = os.path.join(TEMPORARY_FOLDER, unique_filename)
        os.makedirs(TEMPORARY_FOLDER, exist_ok=True)
        file.save(filepath)
        ...
        cleaned_text = clean_text(raw_text)
        matched_keywords = keyword_match(cleaned_text)
        # Use the unique_filename or original filename for name extraction
        ...
        try:
            ...
            print(f"Could turistas parse ATS JSON for {candidate_name}: {e}")
        ...
        candidates.append(candidate) # Add the processed candidate to the list

    # Return the list of all processed candidates
    return jsonify({'candidates': candidates, 'message': f'{len(candidates)} resume(s) analyzed successfully.'})
...
def upload_resume_to_project(project_id):
    ...
    # NEW: List to hold all processed candidate data for the response
    candidates = []
    ...
    for file in files: # Iterate over each uploaded file
        ...
        candidates.append(candidate) # Add the candidate to the list

    # --- Step 4: Update project metadata in Supabase Database (batch update) ---
    projects = load_projects()
    updated_project = None
    if candidates: # Only update if at least one candidate was successfully processed
        log_activity('RESUME_UPLOAD', {'count': len(candidates), 'filenames': [c['filename'] for c in candidates]}) # ⬅️ ADD THIS
        for p in projects:
            if p.get('id') == project_id:
                for candidate in candidates: # Add all successful candidates
    ...
    if save_projects(projects):
        if candidates:
            log_activity('RESUME_UPLOAD', {'count': len(candidates), 'filenames': [c['filename'] for c in candidates]}) # ⬅️ ADD THIS
        return jsonify({'candidates': candidates, 'project': updated_project})
        # CHANGE: Return all uploaded candidates and the updated project
    else:
...
#         user = AUTHORIZED_USERS_TEMP.get(username) # Use the temporary hardcoded list

#         if user and user['password'] == password: # Simple plain text check
...
#             else:
#                 flash("Your account is valid, but your role is not authorized for this portal.", "error")
...
def login():
    ...
    if request.method == "POST":
        ...
        # --- MODIFIED: Check password and if user exists ---
        if user and user['password'] == password: # Simple plain text check
            ...
            # Check if their role is one of the allowed ones
            # REPLACE with THIS BLOCK in index.py
            if user['role'] in allowed_roles:
                # We removed the flash() message from here
                
                next_page = request.args.get('next') or url_for('index')
                
                # Check if we are redirecting to the main index page
                if next_page == url_for('index'):
                    # Add the login_success parameter for the frontend script
                    return redirect(url_for('index', login_success=username.capitalize()))
                
                # If redirecting somewhere else (e.g., a protected page), just redirect
                return redirect(next_page)
            else:
                # Handle other roles like 'guest'
                flash("Your account is valid, but your role is not authorized for this portal.", "error")
                return redirect(url_for('login'))
        # --- END MODIFIED ---
        else:
...
# @login_required # Only authorized recruiters should see the dashboard
@admin_required # <-- MODIFIED
def dashboard_data():
    if not supabase:
        return jsonify({'error': 'Database connection failed.'}), 500
        
    try:
        # 1. Fetch ALL activity logs (or filter for the last 30 days)
        response = supabase.table('activity_log').select('*').order('timestamp', desc=True).limit(500).execute()
        logs = response.data or []
        
        # 2. Fetch project stats
        projects = load_projects() # Use your existing function
        
        # 3. Process logs into required dashboard metrics
        user_activity = defaultdict(lambda: {'logins': 0, 'uploads': 0, 'last_seen': None, 'active': False})
        upload_counts = {'total': 0}
        
        # Monitor active sessions (simple approximation)
        active_users = set()
        ...
        for log in logs:
            ...
            try:
                ...
                print(f"Warning: Invalid timestamp format in log: {timestamp_str}")
                continue
            ...
            elif log['event_type'] == 'RESUME_UPLOAD' or log['event_type'] == 'RESUME_FETCH':
                count = log['details'].get('count', 1)
                user_activity[user]['uploads'] += count
                upload_counts['total'] += count
            
            # Update last seen
            if not user_activity[user]['last_seen'] or timestamp > user_activity[user]['last_seen']:
                user_activity[user]['last_seen'] = timestamp
        
        # Final formatting
        processed_user_activity = []
        for user, data in user_activity.items():
            processed_user_activity.append({
                'username': user,
                'logins': data['logins'],
                'resumes_processed': data['uploads'],
                'last_seen': data['last_seen'].strftime("%Y-%m-%d %H:%M:%S") if data['last_seen'] else 'N/A',
                # In a real app, 'is_active' should be determined by an active session store or recent activity threshold.
                # For this simple log-based approach, we'll keep the list of active_users from the logs.
                'is_active': user in active_users
            })
        
        # Add currently logged-in user if they had no prior log activity
        if session.get('username') and session['username'] not in user_activity:
            processed_user_activity.append({
                'username': session['username'],
                'logins': 1, # Assume at least 1 login for the current session
                'resumes_processed': 0,
                'last_seen': datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S"),
                'is_active': True
            })

        # Project analysis for graph
        project_data = [{
            'title': p['title'],
            # Ensure safe access to nested list property for length
            'total_resumes': len(p.get('resumes', [])),
            'top_kept': len(p.get('top_resumes', []))
        } for p in projects]

        return jsonify({
            'success': True,
            'total_resumes_processed': upload_counts['total'],
            'total_projects': len(projects),
            'user_stats': processed_user_activity,
...
# Add a simple route for the new dashboard page
@app.route("/dashboard")
# @login_required
@admin_required # <-- MODIFIED
def dashboard_page():
