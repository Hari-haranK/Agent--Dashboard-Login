...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # --- END MODIFIED ---
            return f(*args, **kwargs)
        flash("Access denied. Please log in with an authorized account.", "error")
        return redirect(url_for('login', next=request.url))
    return decorated_function
# --- NEW DECORATOR ---
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        flash("Access denied. Please log in with an administrator account.", "error")
        return redirect(url_for('login', next=request.url))
    return decorated_function
# --- END NEW DECORATOR ---
# ==================== MONITORING & LOGGING ====================
...
def get_llm():
    ...
    try:
        ...
        return ChatGoogleGenerativeAI(
            model="gemini-1.5-pro-latest", # Using 1.5 Pro for its large context window
            google_api_key=GOOGLE_API_KEY,
            temperature=0.2, # Kept low, similar to original 0.18
            # Converts system messages to human messages for better compatibility
...
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # MODIFIED: Updated retry logic for Google's API errors
            if "429" in str(e) or "rateLimitExceeded" in str(e).lower() or "resource has been exhausted" in str(e).lower():
                wait_time = (attempt + 1) * 5
                print(f"Rate limit (429) reached. Waiting {wait_time} seconds before retry...")
